...
class TransformerDecoderRoiLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu",
                 self_posembed=None, cross_posembed=None, cross_only=False, num_points=5):
        super().__init__()
        self.cross_only = cross_only
        if not self.cross_only:
            self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward(self, query, key, query_pos, ):
        """
        :param query: B C Pq
        :param key: B C Pk
        :param query_pos: B Pq 3/6
        ...
        #                              key=self.with_pos_embed(key, key_pos_embed),
        #                              value=self.with_pos_embed(key, key_pos_embed), attn_mask=attn_mask)[0]
        new_key = self.conv_roi_1(key.unsqueeze(-1))
        query2 = self.conv_roi_2(torch.cat([query.permute(1,2,0).unsqueeze(-1), new_key], dim=1)).squeeze(-1).permute(2, 0, 1)
...
class TransFusionHead_v5(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(TransFusionHead_v5, self).__init__()
        ...
        if self.use_roi:
            self.bbox_roi_extractor = builder.build_roi_extractor(dict(
        ...
        self.same_assign = same_assign
        self.decouple = decouple
        self.decouple_rw = decouple_rw
        self.decouple_s = decouple_s
        self.decouple_s_u = decouple_s_u
        if self.decouple or self.decouple_s:
            self.d_decoder = copy.deepcopy(self.decoder)
            self.d_prediction_heads = copy.deepcopy(self.prediction_heads)
    ...
    # def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.fuse_img:
            ...
            # if self.fix_guide:
            #     img_feat = img_feat.view(batch_size, self.num_views, num_channel, img_h, img_w)
            #     tmp_img_feat = img_feat.permute(0, 2, 3, 4, 1)
            #     tmp_img_feat = tmp_img_feat.reshape(batch_size, num_channel, img_h, img_w * self.num_views)  # [BS, C, H, n_views*W]
            #     img_feat_collapsed = tmp_img_feat.max(2).values

            #     img_feat = img_feat.view(batch_size, num_channel, img_h, img_w * self.num_views)
            # else:
            ...
            for idx_view in range(self.num_views):
                bev_feat = self.decoder[1 + self.num_decoder_layers + idx_view](bev_feat, img_feat_collapsed[..., img_w * idx_view:img_w * (idx_view + 1)], bev_pos, img_feat_collapsed_pos[:, img_w * idx_view:img_w * (idx_view + 1)])
        ...
        if self.initialize_by_heatmap:
            ...
            if self.fuse_img and not self.no_guide:
            ...
            if self.vis_heatmap:
                if self.vis_num >= 150:
                    exit()
                if self.vis_heatmap == 1:

                    heatmap_vis2 = lidar_feat.abs().mean(1).sigmoid()
                    #heatmap_vis2 = lidar_feat.sigmoid().mean(1)
                    heatmap_vis2 = (heatmap_vis2 - heatmap_vis2.min())/(heatmap_vis2.max()-heatmap_vis2.min())
                    heatmap_vis2 = heatmap_vis2.mul(255).byte().cpu().numpy()

                    colored_heatmap_vis2 = cv2.applyColorMap(heatmap_vis2[0], cv2.COLORMAP_JET)
                    sample_id = img_metas[0]['sample_idx']
                    cv2.imwrite("lidar_vis/"+str(sample_id)+"lidar_feature.jpg", colored_heatmap_vis2)

                    heatmap_vis = dense_heatmap.detach().sigmoid()
                    heatmap_vis = heatmap_vis.max(1)[0]
                    heatmap_vis = (heatmap_vis - heatmap_vis.min())/(heatmap_vis.max()-heatmap_vis.min())
                    heatmap_vis = (heatmap_vis*255).byte().cpu().numpy()
                    colored_heatmap = cv2.applyColorMap(heatmap_vis[0], cv2.COLORMAP_JET) #
                    cv2.imwrite("lidar_vis/"+str(sample_id)+"lidar_heatmap.jpg", colored_heatmap)
                    print("ok")
                    ...
                    heatmap_vis = (heatmap_vis - heatmap_vis.min())/(heatmap_vis.max()-heatmap_vis.min())
                    heatmap_vis = (heatmap_vis*255).byte().cpu().numpy()
                    colored_heatmap = cv2.applyColorMap(heatmap_vis[0], 12)
                    cv2.imwrite("cam_headmap.jpg", colored_heatmap)
                self.vis_num += 1
        ...
        d_query_pos = query_pos + 0.0
        d_query_feat = query_feat + 0.0
        ...
        for i in range(self.num_decoder_layers):
            ...
            if self.decouple or self.decouple_s == 2:

                d_query_feat = self.d_decoder[i](d_query_feat, lidar_feat_flatten, d_query_pos, bev_pos)
                d_res_layer = self.d_prediction_heads[i](d_query_feat)
                d_res_layer['center'] = d_res_layer['center'] + d_query_pos.permute(0, 2, 1)
                d_first_res_layer = d_res_layer
                if not self.fuse_img:
                    d_ret_dicts.append(d_res_layer)
                d_query_pos = d_res_layer['center'].detach().clone().permute(0, 2, 1)
            ...
            if i == 0:
                ...
                for sample_idx in range(batch_size):
                    all_rois.append([LiDARInstance3DBoxes(pred_boxes[sample_idx]['bboxes'][:, :7], box_dim=7)])

                def get_feature_points(bbox, num_points=9):
                    ...
                    if num_points == 9:
                        b_front = front*2 - center
                        b_back = back*2 - center
                        b_left = left*2 - center
                        b_right = right*2 - center
                        points = torch.stack([center, front, back, left, right, b_front, b_back, b_left, b_right],
                                    dim=1)  # [5*num_box, 2]
                    else:
                        points = torch.stack([center, front, back, left, right],
                                    dim=1)  # [5*num_box, 2]
                    return points
                ...
                for sample_idx in range(batch_size):
                    ...
                    all_points = get_feature_points(all_rois[sample_idx][0], num_points=9)
                    xs = (all_points[..., 0] - self.bbox_coder.pc_range[0]
                        ) / self.bbox_coder.voxel_size[0] / self.bbox_coder.out_size_factor
                    ys = (all_points[..., 1] - self.bbox_coder.pc_range[1]
                        ) / self.bbox_coder.voxel_size[1] / self.bbox_coder.out_size_factor 
                    centers = res_layer['center'][sample_idx].permute(1,0).int()
                    corners_x = xs.max(1).values - xs.min(1).values 
                    corners_y = ys.max(1).values - ys.min(1).values
                    corners = torch.stack([corners_x, corners_y], dim=1)
                    radius = torch.ceil(corners.norm(dim=-1, p=2) / 2).int()
                    sigma = (radius * 2 + 1) / 6.0 
                    distance = (centers[:, None, :] - (self.bev_pos.to(centers.device) - 0.5)).norm(dim=-1) ** 2
                    ...
                    # avoid nan
                    attn_mask[attn_mask.sum(1) == 0] += 0.000001

                    tmp_query_feat = self.decoder[i](query_feat[sample_idx:sample_idx+1], lidar_feat_flatten[sample_idx:sample_idx+1], query_pos[sample_idx:sample_idx+1], bev_pos[sample_idx:sample_idx+1], attn_mask=attn_mask.log())
                    if torch.isnan(tmp_query_feat).sum() > 0:
                        import IPython
                        IPython.embed()
                    query_feat_list.append(tmp_query_feat)
        ...
        if self.fuse_img:
            ...
            if self.decouple:
                d_prev_query_feat = d_query_feat.detach().clone()
                d_query_feat = torch.zeros_like(d_query_feat)  # create new container for img query feature
                d_query_pos_realmetric = d_query_pos.permute(0, 2, 1) * self.test_cfg['out_size_factor'] * self.test_cfg['voxel_size'][0] + self.test_cfg['pc_range'][0]
                d_query_pos_3d = torch.cat([d_query_pos_realmetric, d_res_layer['height']], dim=1).detach().clone()
                if 'vel' in d_res_layer:
                    d_vel = copy.deepcopy(d_res_layer['vel'].detach())
                else:
                    d_vel = None
                d_pred_boxes = self.bbox_coder.decode(
                    copy.deepcopy(d_res_layer['heatmap'].detach()),
                    copy.deepcopy(d_res_layer['rot'].detach()),
                    copy.deepcopy(d_res_layer['dim'].detach()),
                    copy.deepcopy(d_res_layer['center'].detach()),
                    copy.deepcopy(d_res_layer['height'].detach()),
                    d_vel,
                )
                d_on_the_image_mask = torch.ones([batch_size, self.num_proposals]).to(d_query_pos_3d.device) * -1
                for sample_idx in range(batch_size if self.fuse_img else 0):
                    d_lidar2img_rt = d_query_pos_3d.new_tensor(img_metas[sample_idx]['lidar2img'])
                    d_img_scale_factor = (
                        d_query_pos_3d.new_tensor(img_metas[sample_idx]['scale_factor'][:2]
                                                if 'scale_factor' in img_metas[sample_idx].keys() else [1.0, 1.0]))
                    d_img_flip = img_metas[sample_idx]['flip'] if 'flip' in img_metas[sample_idx].keys() else False
                    d_img_crop_offset = (
                        d_query_pos_3d.new_tensor(img_metas[sample_idx]['img_crop_offset'])
                        if 'img_crop_offset' in img_metas[sample_idx].keys() else 0)
                    d_img_shape = img_metas[sample_idx]['img_shape'][:2]
                    d_img_pad_shape = img_metas[sample_idx]['input_shape'][:2]
                    d_boxes = LiDARInstance3DBoxes(d_pred_boxes[sample_idx]['bboxes'][:, :7], box_dim=7)
                    d_query_pos_3d_with_corners = torch.cat([d_query_pos_3d[sample_idx], d_boxes.corners.permute(2, 0, 1).view(3, -1)], dim=-1)  # [3, num_proposals] + [3, num_proposals*8]
                    ...
                    if batch_size == 1:  # skip during inference to save time
                        #d_points = d_query_pos_3d_with_corners.T
                        d_points = apply_3d_transformation(d_query_pos_3d_with_corners.T, 'LIDAR', img_metas[sample_idx], reverse=True).detach()

                    else:
                        d_points = apply_3d_transformation(d_query_pos_3d_with_corners.T, 'LIDAR', img_metas[sample_idx], reverse=True).detach()
                    d_num_points = d_points.shape[0]

                    for view_idx in range(self.num_views):
                        d_pts_4d = torch.cat([d_points, d_points.new_ones(size=(d_num_points, 1))], dim=-1)
                        d_pts_2d = d_pts_4d @ d_lidar2img_rt[view_idx].t()

                        d_pts_2d[:, 2] = torch.clamp(d_pts_2d[:, 2], min=1e-5)
                        d_pts_2d[:, 0] /= d_pts_2d[:, 2]
                        d_pts_2d[:, 1] /= d_pts_2d[:, 2]

                        # img transformation: scale -> crop -> flip
                        # the image is resized by img_scale_factor
                        d_img_coors = d_pts_2d[:, 0:2] * d_img_scale_factor  # Nx2
                        d_img_coors -= d_img_crop_offset

                        # grid sample, the valid grid range should be in [-1,1]
                        d_coor_x, d_coor_y = torch.split(d_img_coors, 1, dim=1)  # each is Nx1

                        if d_img_flip:
                            # by default we take it as horizontal flip
                            # use img_shape before padding for flip
                            d_orig_h, d_orig_w = d_img_shape
                            d_coor_x = d_orig_w - d_coor_x

                        d_coor_x, d_coor_corner_x = d_coor_x[0:self.num_proposals, :], d_coor_x[self.num_proposals:, :]
                        d_coor_y, d_coor_corner_y = d_coor_y[0:self.num_proposals, :], d_coor_y[self.num_proposals:, :]
                        d_coor_corner_x = d_coor_corner_x.reshape(self.num_proposals, 8, 1)
                        d_coor_corner_y = d_coor_corner_y.reshape(self.num_proposals, 8, 1)
                        d_coor_corner_xy = torch.cat([d_coor_corner_x, d_coor_corner_y], dim=-1)

                        d_h, d_w = d_img_pad_shape
                        d_on_the_image = (d_coor_x > 0) * (d_coor_x < d_w) * (d_coor_y > 0) * (d_coor_y < d_h)
                        d_on_the_image = d_on_the_image.squeeze()
                        # skip the following computation if no object query fall on current image
                        if d_on_the_image.sum() <= 1:
                            continue
                        d_on_the_image_mask[sample_idx, d_on_the_image] = view_idx

                        # add spatial constraint
                        d_center_ys = (d_coor_y[d_on_the_image] / self.out_size_factor_img)
                        d_center_xs = (d_coor_x[d_on_the_image] / self.out_size_factor_img)
                        d_centers = torch.cat([d_center_xs, d_center_ys], dim=-1).int()  # center on the feature map
                        d_corners = (d_coor_corner_xy[d_on_the_image].max(1).values - d_coor_corner_xy[d_on_the_image].min(1).values) / self.out_size_factor_img
                        d_radius = torch.ceil(d_corners.norm(dim=-1, p=2) / 2).int()  # radius of the minimum circumscribed circle of the wireframe
                        d_sigma = (d_radius * 2 + 1) / 6.0
                        d_distance = (d_centers[:, None, :] - (img_feat_pos - 0.5)).norm(dim=-1) ** 2
                        d_gaussian_mask = (-d_distance / (2 * d_sigma[:, None] ** 2)).exp()
                        d_gaussian_mask[d_gaussian_mask < torch.finfo(torch.float32).eps] = 0
                        d_attn_mask = d_gaussian_mask

                        d_query_feat_view = d_prev_query_feat[sample_idx, :, d_on_the_image]
                        d_query_pos_view = torch.cat([d_center_xs, d_center_ys], dim=-1)
                        d_query_feat_view = self.d_decoder[self.num_decoder_layers](d_query_feat_view[None], img_feat_flatten[sample_idx:sample_idx + 1, view_idx], d_query_pos_view[None], img_feat_pos, attn_mask=d_attn_mask.log())
                        d_query_feat[sample_idx, :, d_on_the_image] = d_query_feat_view.clone()
                self.d_on_the_image_mask = (d_on_the_image_mask != -1)
                d_res_layer = self.d_prediction_heads[self.num_decoder_layers](torch.cat([d_query_feat, d_prev_query_feat], dim=1))
                d_res_layer['center'] = d_res_layer['center'] + d_query_pos.permute(0, 2, 1)
                for d_key, d_value in d_res_layer.items():
                    d_pred_dim = d_value.shape[1]
                    d_res_layer[d_key][~self.d_on_the_image_mask.unsqueeze(1).repeat(1, d_pred_dim, 1)] = d_first_res_layer[d_key][~self.d_on_the_image_mask.unsqueeze(1).repeat(1, d_pred_dim, 1)]
                d_ret_dicts.append(d_res_layer)
                

            prev_query_feat = query_feat.detach().clone()
            ds_query_feat = torch.zeros_like(query_feat)
            if self.decouple_s == 2:
                ds_prev_query_feat = d_query_feat.detach().clone()
            ...
            for sample_idx in range(batch_size if self.fuse_img else 0):
                ...
                for view_idx in range(self.num_views):
                    ...
                    coor_corner_xy_max = coor_corner_xy[on_the_image].max(1).values  / self.out_size_factor_img
                    coor_corner_xy_min = coor_corner_xy[on_the_image].min(1).values / self.out_size_factor_img
                    ...
                    def draw_bbox(xy_min, xy_max):
                        xy_max = xy_max * self.out_size_factor_img
                        xy_min = xy_min * self.out_size_factor_img
                        imgname = img_metas[sample_idx]['filename'][view_idx]
                        ori_img = cv2.imread(imgname)
                        resize_img = cv2.resize(ori_img, (int(img_metas[sample_idx]['pad_shape'][1]),int(img_metas[sample_idx]['pad_shape'][0])))
                        for _i in range(xy_min.shape[0]):
                            tmp_xy_min = xy_min[_i].int().cpu().numpy()
                            tmp_xy_max = xy_max[_i].int().cpu().numpy()
                            cv2.line(resize_img, tuple([tmp_xy_min[0],tmp_xy_min[1]]), tuple([tmp_xy_min[0],tmp_xy_max[1]]) ,(255,0,0),2 )
                            cv2.line(resize_img, tuple([tmp_xy_min[0],tmp_xy_max[1]]), tuple([tmp_xy_max[0],tmp_xy_max[1]]),(255,0,0),2 )
                            cv2.line(resize_img, tuple([tmp_xy_max[0],tmp_xy_min[1]]), tuple([tmp_xy_min[0],tmp_xy_min[1]]),(255,0,0),2 )
                            cv2.line(resize_img, tuple([tmp_xy_max[0],tmp_xy_max[1]]), tuple([tmp_xy_max[0],tmp_xy_min[1]]),(255,0,0),2 )
                            cv2.imwrite("debug"+str(view_idx)+".jpg",resize_img)
                    #draw_bbox(coor_corner_xy_min, coor_corner_xy_max)

                    if self.use_roi:

                        coor_corner_xy_max[:,0][coor_corner_xy_max[:,0]>img_feat.shape[-1]-1] = img_feat.shape[-1]-1
                        coor_corner_xy_max[:,1][coor_corner_xy_max[:,1]>img_feat.shape[-2]-1] = img_feat.shape[-2]-1

                        coor_corner_xy_min[:,0][coor_corner_xy_min[:,0]<0]= 0
                        coor_corner_xy_min[:,1][coor_corner_xy_min[:,1]<0]= 0
                        

                        coor_corner_xy_max_roi = coor_corner_xy_max * self.out_size_factor_img
                        coor_corner_xy_min_roi = coor_corner_xy_min * self.out_size_factor_img
                        rois = torch.cat([torch.zeros(coor_corner_xy_min_roi.shape[0],1).to(coor_corner_xy_min_roi.device), coor_corner_xy_min_roi,coor_corner_xy_max_roi],dim=1)

                        rois_feat = self.bbox_roi_extractor([img_feat[sample_idx:sample_idx + 1, view_idx]], rois)
                        pool_rois_feat = self.avg_pool(rois_feat).squeeze(-1).squeeze(-1)
                        pool_rois_feat = pool_rois_feat.unsqueeze(0)
                        

                        query_feat_view = prev_query_feat[sample_idx, :, on_the_image]
                        query_pos_view = torch.cat([center_xs, center_ys], dim=-1)
                        query_feat_view = self.decoder[self.num_decoder_layers](query_feat_view[None], pool_rois_feat.permute(0, 2, 1), query_pos_view[None])
                        query_feat[sample_idx, :, on_the_image] = query_feat_view.clone()

                    else:
                    # draw_bbox(coor_corner_xy_min.clone(), coor_corner_xy_max.clone()


                        # imgname = img_metas[sample_idx]['filename'][view_idx]
                        # ori_img = cv2.imread(imgname)
                        # resize_img = cv2.resize(ori_img, (int(img_metas[sample_idx]['pad_shape'][1]),int(img_metas[sample_idx]['pad_shape'][0])))
                        # #out_img = cv2.resize(resize_img, (resize_img.shape[1]//self.out_size_factor_img, resize_img.shape[0]//self.out_size_factor_img))
                        # draw_mask = attn_mask.reshape(-1,img_inputs.shape[-2], img_inputs.shape[-1])
                        # draw_mask = draw_mask.max(0)[0] * 250
                        # draw_mask = draw_mask.int().cpu().numpy().astype(np.uint8)
                        # draw_mask = cv2.resize(draw_mask, (resize_img.shape[1], resize_img.shape[0]))[:,:,None]

                        # draw_img = (resize_img*0.6+draw_mask*0.4).astype(np.uint8)
                        # cv2.imwrite("debug"+str(view_idx)+".jpg",draw_img)

                        if self.decouple_s == 2:
                            ds_query_feat_view = ds_prev_query_feat[sample_idx, :, on_the_image]
                            query_pos_view = torch.cat([center_xs, center_ys], dim=-1)
                            ds_query_feat_view = self.d_decoder[self.num_decoder_layers](ds_query_feat_view[None], img_feat_flatten[sample_idx:sample_idx + 1, view_idx], query_pos_view[None], img_feat_pos, attn_mask=attn_mask.log())
                            ds_query_feat[sample_idx, :, on_the_image] = ds_query_feat_view.clone()
                        elif self.decouple_s:
                            ds_query_feat_view = prev_query_feat[sample_idx, :, on_the_image]
                            query_pos_view = torch.cat([center_xs, center_ys], dim=-1)
                            ds_query_feat_view = self.d_decoder[self.num_decoder_layers](ds_query_feat_view[None], img_feat_flatten[sample_idx:sample_idx + 1, view_idx], query_pos_view[None], img_feat_pos, attn_mask=attn_mask.log())
                            ds_query_feat[sample_idx, :, on_the_image] = ds_query_feat_view.clone()
            ...
            if self.decouple_s == 2:
                self.on_the_image_mask = (on_the_image_mask != -1)
                ds_res_layer = self.d_prediction_heads[self.num_decoder_layers](torch.cat([ds_query_feat, ds_prev_query_feat], dim=1))
                ds_res_layer['center'] = ds_res_layer['center'] + query_pos.permute(0, 2, 1)
                for key, value in ds_res_layer.items():
                    pred_dim = value.shape[1]
                    ds_res_layer[key][~self.on_the_image_mask.unsqueeze(1).repeat(1, pred_dim, 1)] = first_res_layer[key][~self.on_the_image_mask.unsqueeze(1).repeat(1, pred_dim, 1)]
                d_ret_dicts.append(ds_res_layer)
            elif self.decouple_s:
                self.on_the_image_mask = (on_the_image_mask != -1)
                ds_res_layer = self.d_prediction_heads[self.num_decoder_layers](torch.cat([ds_query_feat, prev_query_feat], dim=1))
                ds_res_layer['center'] = ds_res_layer['center'] + query_pos.permute(0, 2, 1)
                for key, value in ds_res_layer.items():
                    pred_dim = value.shape[1]
                    ds_res_layer[key][~self.on_the_image_mask.unsqueeze(1).repeat(1, pred_dim, 1)] = first_res_layer[key][~self.on_the_image_mask.unsqueeze(1).repeat(1, pred_dim, 1)]
        ...
        all_return = {
            'd_ret_dicts' : d_ret_dicts
        }

        if self.decouple_s_u:
            new_res['heatmap'] = d_ret_dicts[0]['heatmap']
        return [new_res, all_return]
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts,  **kwargs):
        ...
        loss_dict = self.loss_old(gt_bboxes_3d, gt_labels_3d, (preds_dicts[0],), pred_assign=pred_assign, **kwargs)
        #loss_weigth = [0,0,1]
        loss_weigth = [1,1,1,1]

        if self.decouple_s and not self.decouple_s_u:
            tmp_preds_dicts = (preds_dicts[-1][0]['d_ret_dicts'],)

            decouple_loss_dict = self.loss_old(gt_bboxes_3d, gt_labels_3d, tmp_preds_dicts, just_second=True, pred_assign=pred_assign, **kwargs)
            for k,v in decouple_loss_dict.items():
                if 'loss_cls' in k:
                    loss_dict[k+"_decouple"] = v 
                else:
                    loss_dict[k+"_decouple"] = v * 0
        ...
        for k,v in loss_dict.items():
            loss_dict[k] = v * loss_weigth[0]

        return loss_dict

    @force_fp32(apply_to=('preds_dicts'))
    def loss_old(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, just_second=False, pred_assign=None, **kwargs):
        ...
        if self.initialize_by_heatmap and not just_second:
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False, for_roi=False):
        ...
        decouple_out = None
        if self.decouple:
            decouple_out = preds_dicts[-1][0]['d_ret_dicts'][0]

        preds_dicts = [preds_dicts[0],]
        self.query_labels = self.query_labels
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.decouple:
                decouple_batch_score = decouple_out['heatmap'][..., -self.num_proposals:].sigmoid()
                batch_score = batch_score * decouple_batch_score
